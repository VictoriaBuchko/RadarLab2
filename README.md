...
Перед початком роботи необхідно завантажити та запустити Docker image емулятора радару. Контейнер надає WebSocket сервер на порту 4000, який генерує дані про задетектовані цілі у реальному часі.
...
Створено веб-додаток у вигляді HTML файлу, який підключається до WebSocket сервера та зчитує дані про задетектовані цілі. Для побудови графіку використано бібліотеку **Plotly.js**, яка забезпечує інтерактивне відображення даних у полярних координатах. Інтерфейс реалізовано у темній кольоровій схемі в стилі радарного монітора.
...
де `c = 299 792 458 м/с` — швидкість світла, `t` — час затримки сигналу.
 
![Data processing](screenshots/fig3_data_processing.png)
Рисунок 3 — Обробка отриманих даних (кут, відстань, потужність)
 
### 2.2 Відображення цілей на графіку
 
Кожна ціль відображається як точка на полярному графіку з координатами `(кут, відстань)`. Цілі зберігаються з міткою часу та автоматично видаляються через 6 секунд (TTL = 6000 мс), що створює ефект «хвоста» — сліду руху цілі.
 
![Targets on polar plot](screenshots/fig4_targets.png)
Рисунок 4 — Відображення цілей на полярному графіку
 
### 2.3 Зміна параметрів через API
 
Реалізовано панель керування з можливістю зміни параметрів радару через PUT запит до `http://localhost:4000/config`. Доступні параметри:
...
![API config request](screenshots/fig5_config.png)
Рисунок 5 — Зміна параметрів радару через API запит
 
![Graph update after config](screenshots/fig6_update.png)
Рисунок 6 — Оновлення графіку після зміни параметрів
 
---
 
## 3. Налаштування графіка
 
### 3.1 Відображення відстані в кілометрах
 
Радіальна вісь графіку відображає відстань до цілі в кілометрах, діапазон — від 0 до 200 км. Підписи осей виконано шрифтом `Share Tech Mono` у стилі радарного інтерфейсу.
 
### 3.2 Колірне кодування потужності сигналу та хвіст цілі
 
Колір кожної точки визначається **віком точки** — свіжі точки мають яскравий колір (червоний/оранжевий), а старіші поступово тьмяніють до темно-зеленого. Це створює виразний кольоровий хвіст, який наочно показує траєкторію руху цілі. Використана колірна шкала:
 
| Значення | Колір | Значення |
|----------|-------|----------|
| 0.0 | `#1a4731` | Темно-зелений (стара точка) |
| 0.4 | `#39d353` | Зелений |
| 0.7 | `#ffa657` | Оранжевий |
| 1.0 | `#f85149` | Червоний (нова точка) |
 
![Distance and power display](screenshots/fig7_colors.png)
Рисунок 7 — Відображення відстані та колірне кодування хвоста цілі
...
<details>
<summary>radar.html — повний вихідний код</summary>
...
> Повний код знаходиться у файлі `radar.html` у корені репозиторію.
...
Під час виконання лабораторної роботи було розроблено веб-додаток, який у реальному часі зчитує дані з емульованої вимірювальної частини радару (Docker image) через WebSocket та відображає задетектовані цілі на інтерактивному полярному графіку. Реалізовано обробку даних про відстань (з урахуванням швидкості світла) та потужність сигналу, колірне кодування точок залежно від їх віку для відображення траєкторії руху цілі, а також інтерфейс для зміни параметрів радару через REST API.
